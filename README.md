...
> math, calculus, physics, and STEM explainer videos — straight from a prompt.
...
- 🧰 **Bootstraps the environment** — a Python virtualenv, the animation engine,
  ...
  Linux (`scripts/setup_env.sh`).
- ✍️ **Teaches good scene design** — 3b1b narrative principles plus a tested
  animation cheatsheet.
- 🚫 **Works without LaTeX** — most machines don't have a multi-GB TeX install,
  so it renders math with Pango `Text` + Unicode and documents every
...
- **Python 3.11 or 3.12** (some native dependencies may lack wheels on the very
  newest Python).
...
- Inspired by [3Blue1Brown](https://www.3blue1brown.com/) (Grant Sanderson),
  whose videos defined this visual style of mathematical storytelling.
